import React, { useContext, useState, lazy, Suspense, useEffect, useRef } from 'react';
import { useNavigate, Outlet } from "react-router-dom";
import { Modal, Table, Card, Divider, Button } from 'antd';
...
export function BranchPiece5() {
    ...
    useEffect(() => {
        console.log("INSIDE BRANCH5 PAGE, cur_option changed to: ", cur_option);
    }, [cur_option])
...
function SideSpan({ startPreRun, }) {
    ...
    const guide = <>
        <li>左图：南浔区的生态红线及永久基本农田；</li>
        <li>右图：2015年南浔区土地利用；</li>
    ...
    return (
        <div className={styles.sideSpanContent}>
            <h1 className={styles.sideSpanTitle}>情景分析与评估</h1>

            {guideCard}
            <Divider />
            <Card title="参数设置" className={styles.card}>
                ...
                <div className={`${styles.paramSettingContainer} ${styles.adjust}`}>
                    <span><b>转移成本：</b>默认</span>
...
function SideSpan2({ cur_option, setCurOption, startFormalRun }) {
    ...
    // 如果只传入setCurOption，更新的cur_option不会被sidespan2监听到
    // 因此需要在sidespan2中新增一个cur_option的state，并在sidespan2中更新
    ...
    return (
        <div className={styles.sideSpanContent}>
            ...
            <div className={styles.sideSpan}>当前选项: {option_label}
                <Card title="主导者" className={styles.card}>
                    <SelectGroup selectinfo={base_info}
                ...
                <Card title="参数设置" className={styles.card}>
                    ...
                    {tmp_option && <>
                        <div className={styles.paramSettingContainer}>
                            <span><b>土地需求量：</b>{option_label} 建议值</span>
                            <LandUseModal year='2035' option={tmp_option} />
                        </div>
                        <div className={styles.paramSettingContainer}>
                            <span><b>转移矩阵：</b>
                            {tmp_option !== "base" ? `基于${option_label} 调整` : "基线"}</span>
                            <TransformationModal option={tmp_option} />
                        </div>
                        <Button className={styles.finalButton}
                            onClick={() => { setFinalModal2Visible(true); }}>
                            开始正式模拟
                        </Button>
                        <FinalModal2
                            modal_visible={modal_visible}
...
function SideSpan3({ cur_option, returnStep2 }) {
    ...
    return (
        <div className={styles.sideSpanContent}>
            ...
            <div className={styles.sideSpan}>当前选项: {option_label}
                <Card title="主导者" className={styles.card}>
                    <SelectGroup selectinfo={cur_base_info}
                        usePlaceholder={false} defaultValue={cur_option} />
                </Card>
                <Divider />
                <Card title="参数设置" className={styles.card}>
                    <div className={styles.paramSettingContainer}>
                    ...
                    <div className={styles.paramSettingContainer}>
                        <span><b>转移矩阵：</b>
                        {cur_option !== "base" ? `基于${option_label} 调整` : "基线"}</span>
        ...
        >
            ...
            <div className={styles.finalModal}>
                ...
                <div className={styles.finalModalItem}>
                    <p className={styles.finalModalItemTitle}><b>邻域因子：</b>默认值，不可修改。</p>
                ...
                <div className={styles.finalModalItem}>
                    <p className={styles.finalModalItemTitle}><b>转移成本：</b>默认值，不可修改。</p>
                    <div className={styles.finalModalItemContent}>
                        <ConvertingCostData />
                    </div>
                </div>
    ...
    return (<>
        ...
        >
            ...
            <div className={styles.finalModal}>
                ...
                <div className={styles.finalModalItem}>
                    <p className={styles.finalModalItemTitle}><b>转移矩阵：</b>
...
function Intro() {
    return (
        <>
            <h1 className={styles.introTitle}>情景分析与评估</h1>
            <div className={styles.introContent}>
                <p>基于景观可持续研究中的“格局-服务-福祉”核心关系，本研究通过情景分析，模拟不同利益相关者的态度如何影响土地利用变化。</p>
                <div className={styles.introlist} >
                    <p><b>第一步：</b>根据2010、2015年土地利用数据模拟2019年数据，验证输入参数的有效性。
                        <b>与2019年真实值比较，精度达标后，将进入正式模拟。</b></p>
                    <p><b>第二步：</b>确定利益相关者，开始正式模拟。不同的主体对土地利用的需求不同，因此未来的土地利用将根据主体的需求，朝着不同的方向发展。
                        <b>主体：</b>农民、企业、游客；</p>
...
function NeighborhoodFactorModal() {
    ...
    const intro = <>
        <h2>邻域因子（Neighborhood factor）</h2>
        <p>邻域因子代表了某些土地利用类型向其他土地类型扩展的能力。该因子的范围设置为0-1：
        ...
        <p><b>具体的邻域因子设置表格如下：</b></p>
    </>

    return (
        <>
            <span className={styles.infoOpenButton} onClick={showModal}>(查看详情)</span>
            <Modal
                title="邻域因子 详细设置信息"
                open={visible}
                closable={false}
...
function EnergyModal({ option }) {
    ...
    return (<>
        <Button style={{ backgroundColor: "skyblue", marginBottom: "10px" }}
            className={styles.finalButton}
            onClick={showModal}>情景评估</Button>
        <Modal
            title="能值分析 详细设置信息"
            open={visible}
            closable={false}
